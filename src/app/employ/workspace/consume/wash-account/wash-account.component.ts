...
export class WashAccountComponent implements OnInit {
  ...
  ngOnInit() {
    this.globalCatalogService.setTitle("员工服务/我的工作台/洗衣账户流水");
    this.balance = 0;
    this.queryType = "消费记录";
    this.consumes = [];
    this.initTime();
  ...
  getConsumeInfo(pageNo: number) {
    ...
    this.workspaceMydeskService.getUserConsumeList("laundry", date,this.pageNo, this.pageSize)
      .subscribe(data => {
        if(this.errorResponseService.errorMsg(data)){
  ...
  getUserRechargeList(pageNo: number) {
    ...
    this.workspaceMydeskService.getUserRechargeList("laundry",date,this.pageNo, this.pageSize)
      .subscribe(data => {
        if(this.errorResponseService.errorMsg(data)){
          this.consumes = data.data.infos;
          this.total = data.data.total;
