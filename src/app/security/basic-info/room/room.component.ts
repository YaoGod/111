...
export class RoomComponent implements OnInit {
      ...
      .subscribe(data =>{
        if(this.errorVoid.errorMsg(data.status)) {
          ...
          for( var i = 0;i<this.copyRooms.length;i++){
            this.copyRooms.editStatus = false;
          }
  ...
  initEdit(index: number){
    if(!this.copyRooms[index].editStatus){
      ...
      /*取消编辑*/
      this.copyRooms[index] = JSON.parse(JSON.stringify(this.copyRooms[index]));
  ...
  save(index:number){
    if(this.copyRooms[index].editStatus){
      this.infoBuildingService.updateFloor(this.copyRooms[index])
        .subscribe(data => {
          if(this.errorVoid.errorMsg(data.status)){
            if(data.msg = '更新成功'){
